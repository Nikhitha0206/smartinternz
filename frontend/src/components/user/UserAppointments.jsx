...
const UserAppointments = () => {
  ...
  const getUserAppointment = async () => {
    console.log(userid)
    try {
      const res = await axios.get('http://localhost:5000/api/users/getuserappointments', {
        headers: {
  ...
  const getDoctorAppointment = async () => {
    console.log(userid)
    try {
      const res = await axios.get('http://localhost:5000/api/doctors/getdoctorappointments', {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
        params: {
  ...
  const handleStatus = async (userid, appointmentId, status) => {
    try {
      const res = await axios.post('http://localhost:5000/api/doctors/handlestatus', {
        userid,
        appointmentId,
        status,
      },
  ...
  const handleDownload = async (url, appointId) => {
    try {
      const res = await axios.get('http://localhost:5000/api/doctors/getdocumentdownload', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem("token")}`,
        },
        params: { appointId },
